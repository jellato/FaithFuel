      ...
      <div className="mb-12 mt-8 flex flex-col items-start gap-8 xl:mb-60 xl:flex-row xl:justify-between xl:gap-0">
        ...
        <form className="flex w-full flex-col gap-8 xl:w-[473px]" onSubmit={handleSubmit}>
          <div className="flex flex-col">
            <div className="relative">
              <textarea
                ...
                autoComplete="off"
                placeholder={`Take a few moments to briefly jot down details, thoughts, and feelings about your life. Don't worry, there are no wrong answers...`}
              ...
              <div
                className={`caption-s absolute bottom-4 right-3 ${
                  input.length > 500 ? "text-red-500" : "text-slate-9"
                }`}
              >
                <span>{input.length}</span>/500 remaining
              </div>
            </div>
            {/* Error Message */}
          ...
          <div className="flex flex-col gap-3">
            ...
            >
              {isLoading ? "Generating your devotional..." : "Generate Devotional"}
            </Button>
            {input && completion ? (
              <Button
                variant="secondary"
        ...
        <div className="h-full w-full rounded-[4px] border border-slate-6 bg-slate-2 p-1 justify-center md:p-10 xl:h-[800px] xl:w-[640px] overflow-y-auto">
          {completion ? (
              <p className="body" dangerouslySetInnerHTML={{ __html: completion}} ></p>
          ) : (
              <h3 className="body-l-semibold text-center text-slate-9">
                  Your AI Pastoral response will appear here!
              </h3>
          )}
        </div>
      </div>
